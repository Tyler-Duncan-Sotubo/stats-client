...
interface SongFiltersProps {
  query: Partial<SongQuerySchema>;
  onUpdate: (updates: Partial<SongQuerySchema>) => void;
}

export function SongFilters({ query, onUpdate }: SongFiltersProps) {
  const hasFilters =
    query.search ||
  ...
  return (
    <div className="flex flex-wrap gap-3 items-center">
      ...
      <Select
        ...
        onValueChange={(v) =>
          onUpdate({ explicit: v === "all" ? undefined : v === "true" })
        }
      >
        <SelectTrigger className="h-9 w-36">

...
export function CompareView() {
  ...
  const [leftBrowse, setLeftBrowse] = useState<BrowseArtist | null>(null);
  const [rightBrowse, setRightBrowse] = useState<BrowseArtist | null>(null);
  const [leftArtist, setLeftArtist] = useState<PublicArtist | null>(null);
  const [rightArtist, setRightArtist] = useState<PublicArtist | null>(null);
  ...
  useEffect(() => {
    ...
    if (left) loadArtistBySlug(left, "left");
    if (right) loadArtistBySlug(right, "right");
  }, []);

  async function loadArtistBySlug(slug: string, side: "left" | "right") {
    ...
    try {
      ...
      const browse: BrowseArtist = {
        ...
        isAfrobeats: full.isAfrobeats,
        totalStreams: full.totalStreams,
        monthlyListeners: full.monthlyListeners,
      };
      if (side === "left") {
        setLeftBrowse(browse);
        setLeftArtist(full);
      } else {
        setRightBrowse(browse);
        setRightArtist(full);
      }
    } finally {
  ...
  function clearLeft() {
    setLeftBrowse(null);
    setLeftArtist(null);
    updateUrl(null, rightBrowse?.slug);
  }

  function clearRight() {
    setRightBrowse(null);
    setRightArtist(null);
    updateUrl(leftBrowse?.slug, null);
  }
          ...
          <div className="flex flex-col gap-2 w-full max-w-sm">
            ...
            {SUGGESTED.map((s) => (
              <button
                key={s.label}
                onClick={() => {
                  loadArtistBySlug(s.left, "left");
                  loadArtistBySlug(s.right, "right");
                  updateUrl(s.left, s.right);
                }}

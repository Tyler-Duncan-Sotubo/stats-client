...
export function ArtistCard({ artist }: { artist: BrowseArtist }) {
  return (
    ...
    >
      {/* Image */}
      <div className="relative mb-4 h-32 w-32 overflow-hidden rounded-full bg-muted ring-2 ring-border/50 group-hover:ring-primary/20 transition-all duration-200">
        {artist.imageUrl ? (
          <Image
            src={artist.imageUrl}
            alt={artist.name}
